# LRU Cache [#146](https://leetcode.com/problems/lru-cache/description/) 💾✨
...
* `get(key)` — return the value of the key if it exists in the cache, otherwise return `-1`. Accessing an entry marks it as most recently used.
...
You keep grabbing new snacks throughout the day, but there’s a rule:

#### **If you want to put a new snack in and the backpack is full, you must throw out the snack you haven’t touched for the longest time.**

That’s an **LRU Cache** : *Least Recently Used*.

* Each “snack” = a piece of data the cache stores
* The backpack = the limited-size cache
* “Haven’t touched in a while” = data that hasn’t been accessed recently
* Throwing it out to make room = eviction policy

So an **LRU cache** is like a backpack with a strict teacher inside saying:

> “You’re not eating those stale crackers. Out they go! Make room for the new cookies you actually want!”

It keeps whatever you’ve used **most recently**, and gently tosses the stuff you’ve forgotten about.


<details>
  <summary><p>📖 Wikipedia Article</p></summary>
...
1. Fast lookup of a key → use a hash map (unordered_map).
2. Fast update of "recency" ordering (move a node to the front or end when accessed) → use a doubly linked list.

Put together:

* Hash map: `key -> pointer to linked list node` for O(1) lookup.
* Doubly linked list: store nodes in LRU → MRU order (or vice versa). Removing and inserting nodes is O(1).
...
We want `O(1)` average time per operation. That’s where hashmap + doubly linked list shines.
...
3. `get(1)` → returns 10
   ...
   * mark 1 MRU → cache now: 2 (LRU), 1 (MRU)
4. `put(3, 30)`

   * capacity exceeded → evict LRU (2)
   * cache: 1, 3
5. `get(2)` → -1 (evicted)
6. `get(3)` → 30, mark 3 MRU
...
* **Store indices vs nodes:** we must store pointers/iterators to list nodes in the map. Storing values alone won’t allow O(1) repositioning.
...
* **Thread safety:** not provided. You’d need locks if using from multiple threads.
...
### 〽️ Alternative implementation using `std::list` (short & idiomatic)
