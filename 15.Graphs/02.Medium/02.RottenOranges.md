# 🍊 994. Rotting Oranges - Multi-Source BFS [[Leetcode](https://leetcode.com/problems/rotting-oranges/)]

> This problem is basically **BFS on a grid**, but with multiple starting points.
> Think zombie outbreak 🧟 - all rotten oranges infect their neighbors at the same time.
...
This is literally BFS timing simulation ⏳
...
4. If all oranges rot → return time.
...
Let’s walk through your solution carefully.
...
If grid is empty → no oranges → return -1.
...
* `totOranges` → counts fresh + rotten (non-zero cells)
* `rottenOranges` → how many we process during BFS
* `time` → minutes elapsed



### 🟢 Step 3: Push All Rotten Oranges (Multi-Source Setup)
...
If some oranges were unreachable → return -1.



## 📊 Complexity Analysis

Let `n × m` be grid size.
...
## 🔥 Cleaner Version (Slight Improvement)
...
* Rumor spreading in network 👥
* Flood filling in image processing 🎨
...
* Walls and Gates
* Fire Spread / [Flood Fill](03.FloodFill.md)
* Distance from nearest source
* Rotten oranges variant
* [01 Matrix](06.01Matrix.md) problem
...
Because we need minimum time → shortest layer expansion → BFS.
...
* Grid → Graph
* Infection → BFS expansion
* Time → BFS level
* Multiple sources → Push all initial nodes
